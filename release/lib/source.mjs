 ...
 * places where this contract infers something from a file it did not write, so
...
/**
 * Exactly one `export const NAME[: type] = [<integers>];` — or an error.
 *
 * Used for SUPPORTED_QUOTE_POLICY_VERSIONS, which is the list the client acts on.
 * The same refusal of ambiguity as `readIntConstant`: a duplicated, renamed or
 * computed array (anything that is not a literal list of integers) fails the stamp
 * rather than recording a list the code does not hold. An empty list is refused
 * too — a client that supports no policy version is not a state to certify.
 */
export function readIntArrayConstant(source, name) {
  const matches = [...source.matchAll(new RegExp(`^export const ${name}(?::[^=\\n]+)?\\s*=\\s*\\[([\\d,\\s]*)\\]\\s*;`, 'gm'))];
  if (matches.length !== 1) {
    throw new Error(`expected exactly one \`export const ${name}\` integer-array literal, found ${matches.length}`);
  }
  const values = matches[0][1].split(',').map((v) => v.trim()).filter((v) => v.length > 0);
  if (values.length === 0) throw new Error(`\`${name}\` is an empty list`);
