...
describe('RolePermissionsService', () => {
  ...
  describe('getGrid', () => {
    it('GETs restaurant-setup/role-permissions/ scoped by restaurant and parses res.data into rows', (done) => {
      ...
      service.getGrid('rest-1').subscribe((rows) => {
        expect(api.get).toHaveBeenCalledWith(null, 'restaurant-setup/role-permissions/', { restaurant: 'rest-1' });
  ...
  describe('saveRole', () => {
    it('PUTs {restaurant, role, modules} to restaurant-setup/role-permissions/', () => {
      ...
      expect(api.postPatch).toHaveBeenCalledWith(
        'restaurant-setup/role-permissions/',
