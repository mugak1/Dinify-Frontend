 ...
 *   3. a known runtime advisory blocks, a tooling moderate is triage (not zero), an error
 *      body, an empty body, an unparseable body, an uncounted graph and an
 *      exit-status/body contradiction are each incomplete.
...
const meta = (vulns) => ({ vulnerabilities: { info: 0, low: 0, moderate: 0, high: 0, critical: 0, total: vulns }, dependencies: { total: 2 } });
...
const report = (vulnerabilities) => JSON.stringify({ auditReportVersion: 2, vulnerabilities, metadata: meta(Object.keys(vulnerabilities).length) });
const decide = (run) => {
...
export function selfTest() {
  ...
  expect('runtime advisory blocks', { status: 1, stdout: report({ shipped: { via: [advisory('shipped', 'low', '<3.0.0')], nodes: ['node_modules/shipped'] } }) }, 'blocking');
  expect('tooling moderate is triage, not zero', { status: 1, stdout: report({ tool: { via: [advisory('tool', 'moderate', '<2.0.0')], nodes: ['node_modules/tool'] } }) }, 'within_policy', (r) => r.counts.triageRequired === 1);
  ...
  expect('status 0 with findings is incomplete', { status: 0, stdout: report({ tool: { via: [advisory('tool', 'high', '<2.0.0')], nodes: ['node_modules/tool'] } }) }, 'incomplete');
