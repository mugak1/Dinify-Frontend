...
## 9. HTTP Caching Policy (Media Images vs API Responses)

**Status:** Frontend side implemented; backend response-header contract still
needs confirmation.

**Goal:** Make repeat diner-app visits fast (reuse menu-item images from the
browser cache) while keeping menu data — prices, discounts, `in_stock`,
`available` — perfectly fresh, and never reusing order/session/payment
responses from any cache.

### Where images come from

`MenuItem.image` and `MenuItem.image_thumbnail` are backend-relative paths
(`src/app/_models/app.models.ts:133-134`). The diner-app concatenates them
onto `environment.apiUrl` (`src/app/_shared/utils/image-utils.ts:27-39`,
`src/app/_common/common-image/common-image.component.ts:17`) and renders
them via `<img>` (`ImageWithSkeletonComponent`,
`CommonImageComponent`). They are therefore served by the API origin
(`https://api.dinifyapp.com` in production), **not** Firebase Hosting and
**not** a CDN — only the backend can set their response headers.

### Frontend-side controls (already implemented in this repo)

1. **Firebase Hosting** (`firebase.json`):
   - `index.html` → `Cache-Control: no-cache` (always revalidate the SPA shell).
   - `*.js`, `*.css` → `public, max-age=31536000, immutable` (Angular hashes
     these per build, `outputHashing: "all"`).
   - `*.{png,jpg,jpeg,webp,gif,svg,ico,avif}` → `public, max-age=86400,
     must-revalidate` (covers bundled assets such as logos and icons; these
     are not fingerprinted, so a moderate cache with revalidation is the
     correct ceiling).

2. **`CacheControlInterceptor`** (`src/app/_helpers/cache-control.interceptor.ts`):
   adds `Cache-Control: no-store` and `Pragma: no-cache` request headers to
   every HTTP GET targeting `environment.apiUrl`. This keeps menu JSON,
   order/journey GETs, payment-detail GETs, and table/reservation GETs out of
   every cache layer (browser HTTP cache, intermediate proxies) regardless
   of what response headers the backend sends.

3. **No service worker / PWA cache.** The diner app has no
   `@angular/service-worker` dependency and no `ngsw-config.json`. If a
   service worker is ever introduced, the policy should be:
   - menu-item images → `CacheFirst` (1y, with the route handler ignoring
     query-string cache-busting if the backend uses content-hashed paths) or
     `StaleWhileRevalidate` if filenames are stable;
   - `orders/journey/show-menu/` → `NetworkOnly`;
   - all `orders/`, `finances/`, `users/auth/`, and table-service endpoints
     → `NetworkOnly`.
   Do not introduce a generic API runtime cache.

### Backend-side contract (needs confirmation / implementation)

The frontend needs the backend (and any reverse proxy or object-storage
origin in front of media) to honour the following response headers:

| Response | Required `Cache-Control` | Notes |
|----------|--------------------------|-------|
| Menu-item images (`MenuItem.image`, `MenuItem.image_thumbnail`) and any other diner-visible media | `public, max-age=31536000, immutable` | Safe **only** if the URL changes whenever the image bytes change. The frontend has no `updated_at` on `MenuItem` and cannot append `?v=…` itself, so the backend MUST rotate the stored path (content-hashed filename, or `?v=<hash>` it adds itself, or a new object key) on every upload / re-upload. |
| Restaurant `logo`, `cover_photo` | Same as above (`public, max-age=31536000, immutable`) | Same rotation requirement. |
| Menu JSON (`orders/journey/show-menu/`) | `no-store` (or at minimum `no-cache, must-revalidate`) | Prices, discounts, stock, availability change at any time. |
| Order / journey / payment GETs (`orders/journey/*`, `orders/journey/order-details/`, `orders/journey/payment-details/`) | `no-store, private` | Session/order-sensitive — must never end up in a shared cache. |
| Auth endpoints (`users/auth/*`, token refresh) | `no-store, private` | Contains tokens. |
| Table-service / reservation reads | `no-store` | State changes constantly during service. |

**Action needed:**
1. Confirm that the media-serving layer (Django `MEDIA_URL`, plus any nginx /
   Apache / S3 / CloudFront origin in front of it) sets
   `Cache-Control: public, max-age=31536000, immutable` for `MenuItem.image`
   and `MenuItem.image_thumbnail` responses.
2. Confirm that uploads rotate the stored filename / object key (content
   hash is the standard approach) so the URL changes whenever the image
   bytes change. Without this, the immutable cache is unsafe — a re-upload
   under the same filename would leave diners on stale images for up to a
   year.
3. Confirm that the API sets `Cache-Control: no-store` (or at minimum
   `no-cache, must-revalidate`) on all JSON responses. The frontend
   interceptor is defensive; the response-side header is the canonical fix.

### Acceptance test

- Open the diner app, load a menu, hard-refresh. The first menu-item image
  request returns `200 OK` from the network. The second visit (same image)
  returns `200 (from disk cache)` or `304 Not Modified`.
- The `orders/journey/show-menu/` request shows `Cache-Control: no-store` in
  the request headers; a network round-trip happens on every visit; a price
  edit made in the management portal is reflected on the very next
  diner-app load with no manual reload.
- Order-detail and payment-detail requests never appear with `(from cache)`
  in DevTools.
...
| Media / API Cache-Control headers | Frontend in place, backend needs confirmation | No — performance, not correctness |
